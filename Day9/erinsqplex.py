...
engine = sqlalchemy.create_engine('sqlite:////users/erinrossiter/Dropbox/Summer2016/pythoncourse2016/Day9/players.db', echo=True)

## this is saying, I will be setting up classes
...
## duke is just an instance of class Team
## not in database yet
duke = Team('Duke')

## now we can work with all the players that
## are stored in sql... we didn't have them in
## python environment before
...
#equivalently 
## joining the player table to the team table 
...
session.dirty #tells you what has changed?
session.commit()
session.dirty
# 
